...
$create = mysqli_query($con, "select email from teachers where name = '$user'");
...
function show_comments($comments, $parent_id = -1, $email) {
    $html = '';
    if ($parent_id != -1) {
    ...
    foreach ($comments as $comment) {
        $id = $comment['id'];
        if ($comment['parent_id'] == $parent_id && $comment['email'] == $email) {
            ...
            <div class="comment">
                ...
                <div class="replies">
                ' . show_comments($comments, $comment['id'], $email) . '
                </div>

                <a href="comments.php?search='.$comment['id'].'">Delete</a>
            </div>';
        }
         else if ($comment['parent_id'] == $parent_id) {
            // Add the comment to the $html variable
            $html .= '
            <div class="comment">
                <div>
...
function show_write_comment_form($parent_id = -1) {
    ...
    <div class="write_comment" data-comment-id="' . $parent_id . '">
        <form>
            ...
            <textarea name="content" placeholder="Write your query here..." required></textarea>
...
if (isset($_GET['search'])){
    ...
    $comment_db = mysqli_connect("localhost", "root", "", "phpcomments");

    $comment_id  = $_GET['search'];
    $sql = "delete from comments where id= '$comment_id'";
    $res = mysqli_query($comment_db, $sql);
    if($res){
...
if (isset($_GET['page_id'])) {
    ...
    if (isset($_POST['name'], $_POST['content'])) {
        ...
        $stmt->execute([ $_GET['page_id'], $_POST['parent_id'], $email, $_POST['name'], $_POST['content'] ]);
...
<div class="comment_header">
    <span class="total"><?=$comments_info['total_comments']?> Posts</span>
    <a href="#" class="write_comment_btn" data-comment-id="-1">Create Post</a>
</div>

<?=show_write_comment_form()?>

<?=show_comments($comments, -1, $email)?>
